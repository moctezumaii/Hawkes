...
models=multi.models() 
saveRDS(PN_AllYears7.results2,"newnewmodels.RDS")
#run this to delete junk files
...
models.table
write.csv(PN_AllYears7.table, "modelR2.csv")
#This is to get a specific model:
#to get the model NAG2017.results$Phi$name.model$p$name.model$results$real
###in general model with lowest AIC is explored
modeltoexplore<-PN_AllYears7.results$Phi.time.origin.fl.year.dis.p.time.year$results$real#everytime a $ is written, a menu comes out with all the models
modeltoexplore<-PN_AllYears7.results$Phi.time.origin.fl.year.dis.p.time.fixed$results$real
#to save survival results into an excel file:
write.csv(modeltoexplore, file = "resultsPN_AllYears72.csv")
...
dis.values
Phibydis=covariate.predictions(PN_AllYears7.results$Phi.time.origin.fl.year.dis.p.time.fixed, data= data.frame(dis=dis.values),indices=c(10))
